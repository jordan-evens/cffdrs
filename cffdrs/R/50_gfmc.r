...
#' \code{\link{FireWeatherIndices}} and \code{\link{hffmc}}
...
#' @seealso \code{\link{FireWeatherIndices}}, \code{\link{hffmc}}
...
gfmc <- function(input, GFMCold = 85, batch = TRUE, time.step = 1, roFL = 0.3,
  ...
  for (i in 1:n0){
    ...
    k <- (n * (i - 1) + 1):(n * i)
    MC0 <- GrassFuelMoisture(GFMCold, temp[k], rh[k], ws[k], prec[k], isol[k], time.step, roFL)
    #Eq. 12 - Calculate GFMC
    GFMC0 <- GrassFuelMoistureCode(MC0)
