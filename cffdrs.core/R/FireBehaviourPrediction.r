fctFBP <- Vectorize(function(output, ID, FUELTYPE, HR, LAT, LONG, CBH, SD, SH, CFL, FMC, D0, ELV, DJ, WS, WAZ, SAZ, FFMC, ISI, BUI, PC, PDF, GFL, BUIEFF, GS, CC, ACCEL, THETA)
{
  ...
  CBH <- ifelse(CBH <= 0 | CBH > 50 | is.na(CBH),
                ifelse(FUELTYPE %in%c("C6") & SD > 0 & SH > 0,
  ...
  CFL <- ifelse(CFL <= 0 | CFL > 2 | is.na(CFL), CFLs[FUELTYPE], 
                CFL)
  FMC <- ifelse(FMC <= 0 | FMC > 120 | is.na(FMC), FoliarMoistureContent(LAT,
                                                                         LONG, ELV, DJ, D0), FMC)
  ...
  RSI <- ifelse(isC6, IntermediateSurfaceRateOfSpreadC6(ISI, FMC), 0)
  RSS <- ifelse(isC6, SurfaceRateOfSpreadC6(RSI, BUI), 0)
  RSC <- ifelse(isC6, CrownRateOfSpreadC6(ISI, FMC), 0)
  ...
  if (output == "SECONDARY" | output == "ALL" | output == "S" | 
    ...
    DB <- ifelse(ACCEL == 1, DistanceAtTime(FUELTYPE, BROS, HR, CFB), BROS * HR)
...
FireBehaviourPrediction  <- function(input=NULL, output="Primary") {                                                                                           
  ...
  #                         END
  ############################################################################
  FBP <- do.call(rbind, fctFBP(output, ID, FUELTYPE, HR, LAT, LONG, CBH, SD, SH, CFL, FMC, D0, ELV, DJ, WS, WAZ, SAZ, FFMC, ISI, BUI, PC, PDF, GFL, BUIEFF, GS, CC, ACCEL, THETA))
  row.names(FBP) <- NULL
  return(FBP)
}
