...
#' 
#' @export Ros
Ros <- function(FUELTYPE, ISI, BUI, FMC, SFC, PC, PDF, CC, CBH){
  ...
  RSI <- ifelse(FUELTYPE %in% c("M1"), 
            PC/100 * 
              Ros(rep("C2", length(ISI)),ISI,NoBUI,FMC,SFC,PC,PDF,CC,CBH)
              + (100 - PC) / 100 *
              Ros(rep("D1", length(ISI)),ISI,NoBUI,FMC,SFC,PC,PDF,CC, CBH),
            RSI)
  ...
  ROS <- 
    ifelse(FUELTYPE %in% c("C6"),
      FbpC6(FUELTYPE, ISI, BUI, FMC, SFC, CBH, option = "ROS"),
