...
#' 
#' @export Slope
Slope <- function(FUELTYPE, FFMC, BUI, WS, WAZ, GS, SAZ, FMC, SFC, PC, PDF,
  ...
  RSZ <- Ros(FUELTYPE, ISZ, BUI = NoBUI, FMC, SFC, PC, PDF, CC, CBH)
  ...
  RSZ <- ifelse(FUELTYPE %in% c("M3"), 
           Ros(rep("M3", length(FMC)), ISI = ISZ, BUI = NoBUI, FMC, SFC, 
                    PC, PDF100, CC, CBH), 
           RSZ)
  ...
  RSZ <- ifelse(FUELTYPE %in% c("M4"), 
           Ros(rep("M4", length(FMC)), ISI = ISZ, BUI = NoBUI, FMC, SFC, 
                    PC, PDF100, CC, CBH), 
           RSZ)
