.C6 <- setClass(".C6", contains=".FuelClosed")
          ...
          function(this, output, ID, HR, LAT, LONG, CBH, SD, SH, CFL, FMC, D0, ELV, DJ, WS, WAZ, SAZ, FFMC, ISI, BUI, PC, PDF, GFL, BUIEFF, GS, CC, ACCEL, THETA)
          {
            ...
            if (output == "PRIMARY" | output == "P") {
              ...
              FBP[, c(2:3, 5:ncol(FBP))] <- apply(FBP[, c(2:3, 5:ncol(FBP))], 
                                                  2, function(.x) ifelse(this@name %in% c("WA", "NF"), 
                                                                         0, .x))
              FBP[, "FD"] <- as.character(FBP[, "FD"])
              FBP[, "FD"] <- ifelse(this@name %in% c("WA", "NF"), "NA", FBP[, "FD"])
            }
            ...
            #  and return them.
              ...
              FBP[, 2:ncol(FBP)] <- apply(FBP[, 2:ncol(FBP)], 2, function(.x) ifelse(this@name %in% 
                                                                                       c("WA", "NF"), 0, .x))
            }
            ...
            #  return it.
              ...
              FBP[, c(2:3, 5:ncol(FBP))] <- apply(FBP[, c(2:3, 5:ncol(FBP))],2, function(.x) 
                ifelse(this@name %in% c("WA", "NF"),0,.x))
              FBP[, "FD"] <- as.character(FBP[, "FD"])
              FBP[, "FD"] <- ifelse(this@name %in% c("WA","NF"),"NA",FBP[,"FD"])
            }
            return(list(FBP))
