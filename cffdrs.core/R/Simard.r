...
#' 
#' \code{SimardRateOfSpreadPoint} is used to calculate the rate of spread and
...
#' 
#' \code{SimardRateOfSpreadPoint} allows R users to calculate the rate of spread
#' and direction of a fire across a triangle, given three time measurements and
#' details about the orientation and distance between observational points. The
...
#' degrees). \cr }
#' @return \code{SimardRateOfSpreadPoint} returns a dataframe which includes the
#' rate of spread and spread direction. Output units depend on the user’s inputs
#' for distance (typically meters) and time (seconds or minutes).
#' @author Tom Schiks, Xianli Wang, Alan Cantin
#' @seealso \code{\link{SimardRateOfSpreadLine}},
...
#'                       "LONG3", "LAT3")
#' pros.out1 <- SimardRateOfSpreadPoint(pros.in1)
#' # multiple entries using a dataframe
#' # load the test dataframe for pros
#' data("test_pros")
#' SimardRateOfSpreadPoint(test_pros)
#' 
#' @export SimardRateOfSpreadPoint
SimardRateOfSpreadPoint <- function(input) {
  #############################################################################
  # Description:
  ...
  if (any(exist)){
    errmsg <- paste("cffdrs::SimardRateOfSpreadPoint Column ", colnames(input[exist[TRUE]]), 
...
#' 
#' \code{SimardRateOfSpreadLine} is used to calculate the rate of spread and
#' direction given one set of three point-based observations of fire arrival
#' time. The function requires that the user specify the time that the fire
...
#' many triangles.
#' 
#' \code{SimardRateOfSpreadLine} Allows R users to calculate the rate of spread
#' and direction of a fire across a triangle, given three time measurements and
#' details about the orientation and distance between observational points. The
#' algorithm is based on the description from Simard et al. (1984). See 
#' \code{SimardRateOfSpreadPoint} for more information.
#' 
#' The functions require the user to arrange the input dataframe so that each
#' triangle of interest is identified based on a new row in the dataframe. The
#' input format forces the user to identify the triangles, one triangle per row
...
#' \tab\tab North = 0, West = -90, East = 90 (degrees)\cr 
#' }
#' @return \code{SimardRateOfSpreadLine} returns a dataframe which includes the
#' rate of spread and spread direction. Output units depend on the user’s inputs
#' for distance (typically meters) and time (seconds or minutes).
#' @author Tom Schiks, Xianli Wang, Alan Cantin
#' @seealso \code{\link{SimardRateOfSpreadPoint}},
#' @references 1. Simard, A.J., Eenigenburg, J.E., Adams, K.B., Nissen, R.L.,
#' Deacon, and Deacon, A.G. 1984. A general procedure for sampling and
#' analyzing wildland fire spread.
#' 
...
#' lros.out1 <- SimardRateOfSpreadLine(lros.in1)
#' lros.out1
#' 
#' # multiple entries using a dataframe
...
#' 
#' 
#' @export SimardRateOfSpreadLine
SimardRateOfSpreadLine <- function(input) {
  ...
  if(is.data.frame(input)){
    ...
    # If not, stop execution of function and provide error message
    if (any(exist)){
      errmsg <- paste("cffdrs::SimardRateOfSpreadLine Column ", colnames(input[exist[TRUE]]), 
                      " is required in column list.")
