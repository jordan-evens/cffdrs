...
Fbp  <- function(input=NULL, output="Primary") {                                                                                           
  ...
  CFC <- Tfc(FUELTYPE, CFL, CFB, SFC, PC, PDF, option = "CFC")
  ...
  if (output == "SECONDARY" | output == "ALL" | output == "S" | 
    ...
    #  theta.
    ROSt <- ifelse(ACCEL == 0, ROS, RosT(FUELTYPE, ROS, HR, CFB))
    BROSt <- ifelse(ACCEL == 0, BROS, RosT(FUELTYPE, BROS, HR, CFB))
    FROSt <- ifelse(ACCEL == 0, FROS, FlankRos(ROSt, BROSt, LBt))
    ...
    FCFB <- ifelse(CFL == 0, 0, ifelse(FUELTYPE %in% c("C6"), 
             0, Cfb(FUELTYPE, FMC, SFC, FROS, CBH)))
    BCFB <- ifelse(CFL == 0, 0, ifelse(FUELTYPE %in% c("C6"), 
             0, Cfb(FUELTYPE, FMC, SFC, BROS, CBH)))
    TCFB <- ifelse(CFL == 0, 0, ifelse(FUELTYPE %in% c("C6"), 
             0, Cfb(FUELTYPE, FMC, SFC, TROS, CBH)))
    #Calculate Total fuel consumption for the Flank fire, Back fire and at
    #  angle theta
    FTFC <- Tfc(FUELTYPE, CFL, FCFB, SFC, PC, PDF)
    BTFC <- Tfc(FUELTYPE, CFL, BCFB, SFC, PC, PDF)
    TTFC <- Tfc(FUELTYPE, CFL, TCFB, SFC, PC, PDF)
    ...
    DH <- ifelse(ACCEL == 1, DistT(FUELTYPE, ROS, HR, CFB), ROS * HR)
    DB <- ifelse(ACCEL == 1, DistT(FUELTYPE, BROS, HR, CFB), BROS * HR)
